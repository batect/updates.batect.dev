...
resource "google_monitoring_alert_policy" "bigquery_transfer_errors" {
  ...
  conditions {
    ...
    condition_threshold {
      ...
      // The alignment period below should be 2x the longest interval between runs of any import job.
      aggregations {
        alignment_period     = format("%ds", 2 * module.file_download_events_table.transfer_job_interval_hours * local.seconds_in_hour)
